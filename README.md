...
-  Uniform convergence in machine learning : A class of functions $\left(f_{n}\right)_{n>N}$ is said to be uniformly convergence to $f$ for a data set $E$ if $\forall \epsilon >0$, there exist $n$ such that for $n>N$, $\left|\left|f_{n}-f\right|\right|< \epsilon$.
